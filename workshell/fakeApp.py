...
#### 进入进入home 页
    await homePage.waitForXPath('.//input[@name="keyword"]', visible=True)      # 这里还是需要等待一下
    select = await homePage.xpath('.//input[@name="keyword"]')
    await select[0].type(__wanted)                  # 输入搜索内容
...
#####这里要不使用copy库，深度拷贝一个targePage出来备用，要不就是先把 targePage 当前页的所有用于跳转的 a @ href 取出来放进；
    ...
    totalsize = 0
    try:
        await curPage.waitForXPath('.//p[class="no-res-tip"]', visible=True, timeout=3*1000)
        print("!!!!!!未检索到任何应用，退出程序！！！！")
        return
    except:
        await curPage.waitForXPath("/html/body/div/div[2]/div[1]/div[2]/div/span[3]/i[2]", visible=True)
        totalEle = await curPage.xpath("/html/body/div/div[2]/div[1]/div[2]/div/span[3]/i[2]")
        size = await curPage.evaluate('(element) => element.textContent', totalEle[0])
        totalsize = int(size)
    finally:
        print("###########//共检索到// {} //款应用\n".format(totalsize))
    totalpage = totalsize // 20 + 1
    print("###########共{}页\n".format(totalpage))
    while page <= pages and page <= totalpage:
        if 0 < totalsize <= 20:
            ######      下面的这个字符串格式化坑了我一晚上，差点没看出来原因
            ########    深刻吸取教训
            await curPage.waitForXPath("/ html / body / div / div[2] / div[6] / div[1] / div[2] / div[{}]"
                                       .format(totalsize+2), visible=True)
        else:
            await curPage.waitForXPath("/ html / body / div / div[2] / div[6] / div[1] / div[2] / div[22]",
                                       visible=True)
            totalsize -= 20
        ...
        page += 1
    print("\n成功扫描盘古应用共 {} 页，结束\n".format(pages if pages <= totalpage else totalpage))
...
####### 填充一下
    ...
    worksheet2 = workbook2.add_sheet("盘古应用信息抓取", cell_overwrite_ok=True)
    style = xlwt.XFStyle()  # 初始化样式
    font = xlwt.Font()  # 为样式创建字体
    font.name = 'Times New Roman'
    font.height = 250
    ...
    style.font = font  # 设定样式
    for i in range(len(__wantedArrt)):
        worksheet2.write(0, i, __wantedArrt[i], style)

    data = xlrd.open_workbook('抓取结果.xls')
    table = data.sheet_by_index(0)
    for i in range(1, 12):
        record = ""
        for j in range(1, index):
            if table.cell(j, i).value is not "":
                record = table.cell(j, i).value
                worksheet2.write(j, i, record)
            else:
                worksheet2.write(j, i, record)
